...
public class NatureCoreEntityRenderer extends EntityRenderer<NatureCoreEntity> {
    ...
    public NatureCoreEntityRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }
    ...
    public void render(NatureCoreEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        model.renderToBuffer(pPoseStack, pBuffer.getBuffer(lightning(getTextureLocation(pEntity))), pPackedLight, OverlayTexture.NO_OVERLAY, color2.getRed()/255f, color2.getGreen()/255f, color2.getBlue()/255f, 0.75f);
